            ...
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                <thead class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الصلاحيات</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">المستخدمون</th>
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    @forelse ($roles as $role)
                        ...
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-4 py-3 text-start">
                                @if ($isProtected)
                                    <span class="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">نظامي</span>
                                @else
                                    <span class="rounded-full bg-mist-100 px-2 py-0.5 text-[10px] font-semibold text-mist-500 dark:bg-ink-700 dark:text-mist-400">مخصص</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-mist-500 text-start">{{ $role->permissions_count }}</td>
                            <td class="px-4 py-3 text-start">
                                <span class="inline-flex min-w-8 items-center justify-center rounded-full bg-mist-100 px-2 py-0.5 text-xs font-semibold text-ink-700 dark:bg-ink-700 dark:text-mist-200">
                                    {{ $role->users_count }}
                                </span>
                            </td>
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    @can('tenant.roles.manage')
                                        <a
                                            ...
                                            class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600"
                                        >
                                            تعديل
                                        </a>
                                        @unless ($isProtected)
                    ...
                    @empty
                        <x-ui.table-empty :colspan="6" icon="🔑" message="لا توجد أدوار بعد." />
                    @endforelse
                </tbody>
            </table>
        </div>
