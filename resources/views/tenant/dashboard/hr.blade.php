    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                @can('hr.employees.create')
                    <a href="{{ route('hr.employees.create') }}" class="inline-flex items-center rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300">إضافة موظف</a>
                @endcan
                @can('hr.attendance.view_any')
                    <a href="{{ route('hr.attendance.index') }}" class="inline-flex items-center rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-ink-700 transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600 dark:text-mist-200">سجل الحضور</a>
                @endcan
                @can('tenant.reports.view')
                    <a href="{{ route('tenant.reports.index') }}" class="inline-flex items-center rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-ink-700 transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600 dark:text-mist-200">التقارير</a>
                @endcan
            </div>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <div class="{{ $card }}">
                ...
                <p class="mt-2 font-display text-3xl font-bold text-ink-900 dark:text-ink-50" data-testid="kpi-headcount">{{ $kpis['headcount'] }}</p>
            </div>
            <div class="{{ $card }}">
                <p class="text-sm font-medium text-mist-500 dark:text-mist-400">حاضرون اليوم</p>
                <p class="mt-2 font-display text-3xl font-bold text-emerald-600 dark:text-emerald-400" data-testid="kpi-present-today">{{ $kpis['present_today'] }}</p>
            </div>
            <div class="{{ $card }}">
                <p class="text-sm font-medium text-mist-500 dark:text-mist-400">نسبة الغياب اليوم</p>
                <p class="mt-2 font-display text-3xl font-bold text-ink-900 dark:text-ink-50" data-testid="kpi-absence-rate">{{ $kpis['absence_rate'] }}%</p>
            </div>
            <div class="{{ $card }}">
                <p class="text-sm font-medium text-mist-500 dark:text-mist-400">طلبات إجازة معلّقة</p>
                <p class="mt-2 font-display text-3xl font-bold text-amber-600 dark:text-amber-400" data-testid="kpi-pending-leaves">{{ $kpis['pending_leaves'] }}</p>
            </div>
        </div>
        ...
        <div class="{{ $card }}">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-3 xl:grid-cols-5">
                @foreach ([
                    ['حاضر', $attendanceToday['present'], 'text-emerald-600 dark:text-emerald-400', 'present'],
                    ...
                    <div class="rounded-xl bg-mist-50 p-3 text-center dark:bg-ink-900/40">
                        ...
                        <p class="mt-1 font-display text-2xl font-bold {{ $tone }}" data-testid="attendance-{{ $slug }}">{{ $value }}</p>
                    </div>
                @endforeach
            </div>
            ...
            <div class="{{ $card }} lg:col-span-2">
                <div class="flex items-center justify-between">
                    ...
                    @can('hr.leaves.view_any')
                        <a href="{{ route('hr.leaves.index', ['status' => 'pending']) }}" class="text-xs font-semibold text-emerald-600 hover:underline dark:text-emerald-400">عرض الكل</a>
                    @endcan
                </div>
                    ...
                    @forelse ($pendingLeaves as $leave)
                        <li class="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-mist-50 px-3 py-2 text-sm dark:bg-ink-900/40">
                            ...
                            <div class="flex items-center gap-3 text-xs text-mist-500">
                                ...
                                <span class="rounded-full bg-amber-400/15 px-2 py-0.5 font-bold text-amber-800 dark:text-amber-300">{{ $leave->days_count }} يوم</span>
                            </div>
                        </li>
                    @empty
            ...
            <div class="{{ $card }}">
                ...
                <div class="mt-4">
                    <div class="flex items-end justify-between">
                        <span class="font-display text-3xl font-bold text-ink-900 dark:text-ink-50" data-testid="evaluation-completion">{{ $evaluations['completion_rate'] }}%</span>
                    ...
                    <div class="mt-2 h-2 overflow-hidden rounded-full bg-mist-100 dark:bg-ink-700">
                        <div class="h-full rounded-full bg-emerald-400" style="width: {{ min(100, $evaluations['completion_rate']) }}%"></div>
                    </div>
                </div>
        ...
        <div class="{{ $card }}">
            <div class="flex items-center justify-between">
                ...
                @if ($tasks['overdue'] > 0)
                    <span class="rounded-full bg-danger-solid/10 px-3 py-1 text-xs font-bold text-danger-solid" data-testid="tasks-overdue">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                @foreach (TaskStatus::cases() as $status)
                    <div class="rounded-xl bg-mist-50 p-4 dark:bg-ink-900/40">
                        <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $taskColumnStyles[$status->value] }}">
                            {{ $status->label() }}
                        </span>
                        <p class="mt-2 font-display text-2xl font-bold text-ink-900 dark:text-ink-50" data-testid="tasks-{{ $status->value }}">
                ...
                <ul class="mt-3 space-y-2 text-sm" data-testid="anniversaries">
                    @forelse ($anniversaries as $row)
                        <li class="flex items-center justify-between gap-2">
                            ...
                            <span class="shrink-0 rounded-full bg-emerald-400/15 px-2 py-0.5 text-xs font-bold text-emerald-700 dark:text-emerald-400">
                                {{ $row['years'] }} سنة
                            </span>
                        </li>
                    @empty
