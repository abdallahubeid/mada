@php
    ...
    $typeBadges = [
        ...
        AnnouncementType::Event->value => 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300',
...
<x-layouts.app title="التعميمات والإعلانات">
    ...
    >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">التعميمات والإعلانات</h1>
            ...
            @if ($canManage)
                <button
                    type="button"
                    @click="openCreate()"
                    class="inline-flex items-center justify-center rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 hover:bg-emerald-300"
                >
                    تعميم جديد
                </button>
            @endif
                ...
                <table class="w-full min-w-max text-sm">
                    <thead>
                        <tr class="border-b border-mist-100 text-xs text-mist-500 dark:border-ink-700">
                            ...
                            <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">العنوان</th>
                            ...
                            <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">النشر / الانتهاء</th>
                    ...
                    <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                        @forelse ($announcements as $announcement)
                            ...
                            <tr class="hover:bg-mist-50 dark:hover:bg-ink-700/40">
                                <td class="w-12 px-4 py-3 text-center text-sm tabular-nums text-mist-500">{{ $loop->iteration + ($announcements->currentPage() - 1) * $announcements->perPage() }}</td>
                                <td class="px-4 py-3 text-start">
                                    <p class="font-medium text-ink-900 dark:text-ink-50">
                                        @if ($announcement->is_pinned)
                                            <span class="me-1 text-amber-500">📌</span>
                                        @endif
                                        {{ $announcement->title }}
                                    </p>
                                ...
                                <td class="px-4 py-3 text-start">
                                    <span @class(['rounded-full px-2 py-0.5 text-xs font-semibold', $typeBadges[$announcement->type->value] ?? ''])>
                                        {{ $announcement->type->label() }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    @if ($announcement->isActive())
                                        <span class="rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300">نشط</span>
                                    @else
                                        <span class="rounded-full bg-mist-100 px-2 py-0.5 text-xs font-semibold text-mist-500 dark:bg-ink-700">منتهٍ</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-start text-xs text-mist-500">
                        ...
                        @empty
                            <x-ui.table-empty :colspan="6" icon="📢" message="لا توجد تعميمات بعد." />
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <div class="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-ink-800" @click.outside="close()">
                    ...
                    >
                        ...
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="is_pinned" value="1" class="rounded border-mist-300 text-emerald-500" :checked="!!editing?.is_pinned">
