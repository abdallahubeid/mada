<x-layouts.app title="أعضاء الفريق">
    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">أعضاء الفريق</h1>
        ...
        <form method="GET" action="{{ route('team.index') }}" class="grid gap-3 rounded-2xl border border-mist-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_220px_auto] dark:border-ink-600 dark:bg-ink-800">
            ...
            <button type="submit" class="rounded-xl bg-ink-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-ink-800 dark:bg-ink-700 dark:hover:bg-ink-600">
                تصفية
            </button>
        </form>
            ...
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                <thead class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">العضو</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">البريد</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">القسم</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الدور</th>
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    @forelse ($members as $member)
                        ...
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-4 py-3 text-mist-500 text-start"><x-ui.ltr>{{ $member->email }}</x-ui.ltr></td>
                            <td class="px-4 py-3 text-start">
                                @if ($member->department)
                                    <span class="inline-flex rounded-full bg-mist-100 px-2.5 py-0.5 text-xs font-semibold text-ink-700 dark:bg-ink-700 dark:text-mist-200">
                            ...
                            <td class="px-4 py-3 text-start">
                                <span class="inline-flex rounded-full bg-emerald-400/15 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300">
                                    {{ $roleLabel }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if ($member->is_active)
                                    <span class="inline-flex rounded-full bg-emerald-400/15 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300">نشط</span>
                                @else
                                    <span class="inline-flex rounded-full bg-mist-200 px-2.5 py-0.5 text-xs font-semibold text-mist-600 dark:bg-ink-700 dark:text-mist-300">معطّل</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-center">
                    ...
                    @empty
                        <x-ui.table-empty :colspan="7" icon="👥" message="لا يوجد أعضاء مطابقون للبحث." />
                    @endforelse
                </tbody>
            </table>
        </div>
