    ...
    >
            ...
            <div class="flex flex-col items-start gap-5 sm:flex-row sm:items-center">
                <div class="relative h-20 w-20 shrink-0">
                    <button
                        type="button"
                        @click="openLightbox()"
                        class="group h-20 w-20 overflow-hidden rounded-full border border-mist-200 bg-emerald-400/15 transition ring-offset-2 hover:ring-2 hover:ring-emerald-400/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 dark:border-ink-600 dark:ring-offset-ink-800"
                        title="عرض الصورة"
                    ...
                    <button
                        type="button"
                        @click="pickAvatar()"
                        class="absolute -bottom-1 -end-1 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full border-2 border-white bg-emerald-500 text-ink-950 shadow-md transition duration-200 hover:bg-emerald-400 active:scale-90 dark:border-ink-800"
                        title="تغيير الصورة"
                ...
                <div class="min-w-0 flex-1">
                    <h3 class="font-display text-xl font-semibold text-ink-900 dark:text-ink-50">{{ $user->name }}</h3>
                    <p class="mt-0.5 text-sm text-mist-500 dark:text-mist-400" dir="ltr">{{ $user->email }}</p>
                    <div class="mt-3 flex flex-wrap items-center gap-2">
                        <span class="inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-400">
        ...
        >
            ...
            <div x-show="tab === 'security'" x-cloak class="{{ $cardClass }} space-y-5">
                <div>
                    <h3 class="font-display text-base font-semibold text-ink-900 dark:text-ink-50">الأمان وكلمة المرور</h3>
            ...
            <div class="mt-6 flex items-center justify-end gap-3 border-t border-mist-200 pt-5 dark:border-ink-700">
                <a href="{{ route('profile.edit') }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-mist-600 transition hover:bg-mist-100 dark:text-mist-300 dark:hover:bg-ink-800">إلغاء</a>
                <button type="submit" class="rounded-xl bg-emerald-400 px-5 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300 active:scale-95">
                    حفظ التغييرات
                </button>
            </div>
        </form>
        ...
        >
            ...
            <div class="relative z-10 w-full max-w-lg overflow-hidden rounded-2xl border border-ink-700 bg-ink-900 shadow-2xl">
                ...
                <div class="flex flex-wrap items-center justify-end gap-2 border-t border-ink-700 px-4 py-3">
                    <button
                        ...
                        class="inline-flex items-center gap-2 rounded-xl border border-ink-600 px-4 py-2 text-sm font-semibold text-mist-200 transition hover:border-emerald-400 hover:text-emerald-400"
                    >
                        إعادة ضبط / قص الصورة
                    </button>
                    <button type="button" @click="pickAvatar()" class="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-ink-950 transition hover:bg-emerald-400">
                        اختيار صورة جديدة
                    </button>
                </div>
            </div>
            ...
            <div class="relative z-10 flex w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-ink-700 bg-ink-900 shadow-2xl">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 border-t border-ink-700 px-4 py-3">
                    <div class="flex items-center gap-2">
                        <button type="button" @click="zoom(-0.1)" class="rounded-lg border border-ink-600 px-3 py-1.5 text-xs font-semibold text-mist-200 hover:border-emerald-400 hover:text-emerald-400" title="تصغير">−</button>
                        <button type="button" @click="zoom(0.1)" class="rounded-lg border border-ink-600 px-3 py-1.5 text-xs font-semibold text-mist-200 hover:border-emerald-400 hover:text-emerald-400" title="تكبير">+</button>
                        <button type="button" @click="rotate(-90)" class="rounded-lg border border-ink-600 px-3 py-1.5 text-xs font-semibold text-mist-200 hover:border-emerald-400 hover:text-emerald-400" title="تدوير يسار">↺</button>
                        <button type="button" @click="rotate(90)" class="rounded-lg border border-ink-600 px-3 py-1.5 text-xs font-semibold text-mist-200 hover:border-emerald-400 hover:text-emerald-400" title="تدوير يمين">↻</button>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" @click="applyCrop()" class="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-ink-950 transition hover:bg-emerald-400">
                            تطبيق القص
                        </button>
                    </div>
                </div>
    ...
    <x-slot:scripts>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('veyraProfileAvatar', (config) => ({
