...
<x-layouts.app title="ملف الموظف">
    <div class="mx-auto max-w-5xl space-y-6" x-data="{ activeTab: @js($activeTab), leaveOpen: false }">
        <section class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800 sm:p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="flex items-start gap-4">
                    ...
                    >
                    <div>
                        <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">{{ $employee->full_name }}</h1>
                        <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">{{ $employee->job_title }}</p>
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            @if ($employee->department)
                                <span class="inline-flex rounded-full bg-mist-100 px-2.5 py-0.5 text-xs font-semibold text-mist-700 dark:bg-ink-700 dark:text-mist-200">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @can('hr.attendance.create')
                        @if ($todayAttendance?->check_in === null)
                            <form method="POST" action="{{ route('hr.attendance.check-in') }}">
                                ...
                                <button type="submit" class="rounded-xl border border-emerald-400/40 bg-emerald-400/10 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-400/20 dark:text-emerald-300">
                                    تسجيل حضور
                                </button>
                            </form>
                    ...
                    <a
                        ...
                        class="rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-mist-600 transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600 dark:text-mist-300"
                    >
                        رجوع
                    </a>
                </div>
        ...
        <div class="flex flex-wrap gap-2 border-b border-mist-200 pb-2 dark:border-ink-700">
            <button type="button" @click="activeTab = 'overview'" :class="activeTab === 'overview' ? 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' : 'text-mist-500 hover:bg-mist-100 dark:hover:bg-ink-800'" class="rounded-xl px-4 py-2 text-sm font-semibold transition">نظرة عامة</button>
            <button type="button" @click="activeTab = 'contract'" :class="activeTab === 'contract' ? 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' : 'text-mist-500 hover:bg-mist-100 dark:hover:bg-ink-800'" class="rounded-xl px-4 py-2 text-sm font-semibold transition">العقد النشط</button>
            <button type="button" @click="activeTab = 'attendance'" :class="activeTab === 'attendance' ? 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' : 'text-mist-500 hover:bg-mist-100 dark:hover:bg-ink-800'" class="rounded-xl px-4 py-2 text-sm font-semibold transition">سجل الحضور</button>
            <button type="button" @click="activeTab = 'leaves'" :class="activeTab === 'leaves' ? 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' : 'text-mist-500 hover:bg-mist-100 dark:hover:bg-ink-800'" class="rounded-xl px-4 py-2 text-sm font-semibold transition">🌴 الإجازات</button>
            <button type="button" @click="activeTab = 'performance'" :class="activeTab === 'performance' ? 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' : 'text-mist-500 hover:bg-mist-100 dark:hover:bg-ink-800'" class="rounded-xl px-4 py-2 text-sm font-semibold transition">🎯 التقييمات</button>
            @can('hr.assets.view_any')
                <button type="button" @click="activeTab = 'assets'" :class="activeTab === 'assets' ? 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' : 'text-mist-500 hover:bg-mist-100 dark:hover:bg-ink-800'" class="rounded-xl px-4 py-2 text-sm font-semibold transition">العُهد والأصول 💼</button>
            @endcan
        </div>

        <div x-show="activeTab === 'overview'" x-cloak class="space-y-4">
            <div class="grid gap-4 sm:grid-cols-2">
                <section class="space-y-3 rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
            ...
            <section class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                <h2 class="text-sm font-semibold text-ink-900 dark:text-ink-50">السيرة الذاتية</h2>
                <div class="mt-3 text-sm">
                    @if ($employee->cvUrl())
                        <a href="{{ $employee->cvUrl() }}" target="_blank" rel="noopener" class="font-semibold text-emerald-600 hover:underline dark:text-emerald-400">
                ...
                <section class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                    ...
                    <ul class="mt-3 space-y-2 text-sm text-mist-600 dark:text-mist-300">
                        @foreach ($employee->subordinates as $subordinate)
                            <li>
                                <a href="{{ route('hr.employees.show', $subordinate) }}" class="font-medium text-emerald-600 hover:underline dark:text-emerald-400">
            ...
            <section class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                ...
                @if ($activeContract)
                    ...
                    @can('hr.contracts.update')
                        <a href="{{ route('hr.contracts.edit', $activeContract) }}" class="mt-4 inline-flex text-sm font-semibold text-emerald-600 hover:underline dark:text-emerald-400">تعديل العقد</a>
                    @endcan
                @else
                    <p class="mt-3 text-sm text-mist-500">لا يوجد عقد نشط لهذا الموظف.</p>
                    @can('hr.contracts.create')
                        <a href="{{ route('hr.contracts.create', ['employee_id' => $employee->id]) }}" class="mt-3 inline-flex text-sm font-semibold text-emerald-600 hover:underline dark:text-emerald-400">إضافة عقد</a>
                    @endcan
                @endif
            </section>
        </div>
                ...
                <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                    <thead class="bg-mist-50 dark:bg-ink-900">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الحضور</th>
                            <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الانصراف</th>
                            <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الساعات</th>
                    ...
                    <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                        @forelse ($attendances as $attendance)
                            <tr>
                                ...
                                <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $attendance->date?->format('Y-m-d') }}</x-ui.ltr></td>
                                <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $attendance->check_in?->format('H:i') ?? '—' }}</x-ui.ltr></td>
                                <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $attendance->check_out?->format('H:i') ?? '—' }}</x-ui.ltr></td>
                                <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $attendance->workedHoursLabel() }}</x-ui.ltr></td>
                                <td class="px-4 py-3 text-center">
                                    <span @class(['inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold', $attendanceStatusClasses[$attendance->status->value] ?? ''])>
                                        {{ $attendance->status->label() }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <x-ui.table-empty :colspan="6" icon="🕒" message="لا توجد سجلات حضور بعد." />
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div x-show="activeTab === 'leaves'" x-cloak class="space-y-4">
            <div class="flex items-center justify-between gap-3">
                ...
                @can('hr.leaves.create')
                    <button type="button" @click="leaveOpen = true" class="rounded-xl bg-emerald-400 px-3 py-1.5 text-xs font-semibold text-emerald-900 shadow-glow hover:bg-emerald-300">
                        طلب إجازة جديد
                    </button>
                @endcan
            </div>
            ...
            <div class="grid gap-3 sm:grid-cols-3">
                @forelse ($leaveBalances as $balance)
                    <div class="rounded-2xl border border-mist-200 bg-white p-4 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                        ...
                        <p class="mt-2 text-2xl font-bold text-emerald-600 dark:text-emerald-400">{{ $balance['remaining'] }}</p>
                    ...
                    <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                        @forelse ($leaveRequests as $leaveRequest)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-center">{{ $leaveRequest->status->label() }}</td>
                            </tr>
                        @empty
            ...
            >
                <div class="w-full max-w-lg rounded-2xl border border-mist-200 bg-white p-5 shadow-xl dark:border-ink-600 dark:bg-ink-800" @click.outside="leaveOpen = false">
                    ...
                    <form method="POST" action="{{ route('hr.leaves.requests.store') }}" class="mt-4 space-y-3">
                        ...
                        <button type="submit" class="w-full rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900">إرسال الطلب</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'performance'" x-cloak class="space-y-4">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                @can('hr.evaluations.access')
                    <a href="{{ route('hr.evaluations.index') }}" class="text-sm font-semibold text-emerald-600 hover:underline dark:text-emerald-400">لوحة التقييمات</a>
                @endcan
            </div>
            <div class="w-full overflow-x-auto rounded-2xl border border-mist-200 bg-white shadow-sm dark:border-ink-600 dark:bg-ink-800">
                <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                    ...
                    <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                        @forelse ($evaluations as $evaluation)
                            <tr>
                                <td class="w-12 px-4 py-3 text-center text-sm tabular-nums text-mist-500">{{ $loop->iteration }}</td>
                                <td class="px-4 py-3 font-medium text-start"><x-ui.ltr>{{ $evaluation->period_key }}</x-ui.ltr></td>
                                <td class="px-4 py-3 text-start">{{ $evaluation->period_type->label() }}</td>
                                <td class="px-4 py-3 tabular-nums font-semibold text-start"><x-ui.ltr>{{ $evaluation->rating ?? '—' }}</x-ui.ltr></td>
                                <td class="px-4 py-3 text-start">{{ $evaluation->evaluator?->full_name ?? '—' }}</td>
                                <td class="px-4 py-3 text-center">{{ $evaluation->status->label() }}</td>
                            </tr>
                        @empty
                            <x-ui.table-empty :colspan="6" icon="⭐" message="لا توجد تقييمات بعد." />
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        @can('hr.assets.view_any')
            <div x-show="activeTab === 'assets'" x-cloak class="space-y-4">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('tenant.assets.employee', $employee) }}" class="text-sm font-semibold text-emerald-600 hover:underline dark:text-emerald-400">عرض صفحة العهدة الكاملة</a>
                </div>
                        ...
                        <table class="w-full min-w-max text-sm">
                            <thead>
                                <tr class="border-b border-mist-100 text-xs text-mist-500 dark:border-ink-700">
                                    ...
                                    <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الأصل</th>
                                    <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">التصنيف</th>
                                    <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">تاريخ الإسناد</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                                @forelse ($activeAssetAssignments as $assignment)
                                    <tr>
                                        <td class="w-12 px-4 py-3 text-center text-sm tabular-nums text-mist-500">{{ $loop->iteration }}</td>
                                        <td class="px-4 py-3 font-mono text-xs text-start"><x-ui.ltr>{{ $assignment->asset?->asset_code }}</x-ui.ltr></td>
                                        <td class="px-4 py-3 text-start">{{ $assignment->asset?->name }}</td>
                                        <td class="px-4 py-3 text-start">{{ $assignment->asset?->category->label() }}</td>
                                        <td class="px-4 py-3 text-start"><x-ui.ltr>{{ $assignment->assigned_at?->format('Y-m-d') }}</x-ui.ltr></td>
                                    </tr>
                                @empty
                                    <x-ui.table-empty :colspan="5" icon="💼" message="لا توجد أصول مسندة حالياً." />
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="w-full min-w-max text-sm">
                            ...
                            <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                                @forelse ($assetAssignmentHistory as $assignment)
                                    <tr>
                                        <td class="w-12 px-4 py-3 text-center text-sm tabular-nums text-mist-500">{{ $loop->iteration + ($assetAssignmentHistory->currentPage() - 1) * $assetAssignmentHistory->perPage() }}</td>
                                        <td class="px-4 py-3 text-start">{{ $assignment->asset?->asset_code }} — {{ $assignment->asset?->name }}</td>
                                        <td class="px-4 py-3 text-start"><x-ui.ltr>{{ $assignment->assigned_at?->format('Y-m-d') }}</x-ui.ltr></td>
                                        <td class="px-4 py-3 text-start"><x-ui.ltr>{{ $assignment->returned_at?->format('Y-m-d') ?? 'نشط' }}</x-ui.ltr></td>
                                        <td class="px-4 py-3 text-center">
                                ...
                                @empty
                                    <x-ui.table-empty :colspan="5" icon="💼" message="لا يوجد سجل إسنادات." />
                                @endforelse
                            </tbody>
                        </table>
                    </div>
