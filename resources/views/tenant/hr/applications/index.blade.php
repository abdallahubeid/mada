@php
    ...
    $statusClasses = [
        ...
        ApplicationStatus::Accepted->value => 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300',
...
<x-layouts.app title="طلبات التقديم">
    <div class="space-y-6">
        <div>
            <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">طلبات التقديم</h1>
            ...
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                <thead class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">المتقدم</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">الوظيفة</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">البريد</th>
                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400 text-start">المرحلة</th>
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    @forelse ($applications as $application)
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-4 py-3 font-medium text-ink-900 dark:text-ink-50 text-start">{{ $application->applicant_name }}</td>
                            <td class="px-4 py-3 text-mist-500 text-start">{{ $application->jobPosting?->title ?? '—' }}</td>
                            <td class="px-4 py-3 text-mist-500 text-start"><x-ui.ltr>{{ $application->email }}</x-ui.ltr></td>
                            <td class="px-4 py-3 text-start">
                                <span @class(['inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold', $statusClasses[$application->status->value] ?? ''])>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    @can('hr.applications.view')
                                        <a href="{{ route('hr.applications.show', $application) }}" class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold hover:border-emerald-400 dark:border-ink-600">عرض</a>
                                    @endcan
                    ...
                    @empty
                        <x-ui.table-empty :colspan="6" icon="📥" message="لا توجد طلبات تقديم بعد." />
                    @endforelse
                </tbody>
            </table>
        </div>
