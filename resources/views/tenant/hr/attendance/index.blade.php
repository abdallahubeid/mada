...
<x-layouts.app title="سجل الحضور والغياب">
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">سجل الحضور والغياب</h1>
            ...
            <section class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                ...
                <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($employees->take(12) as $employee)
                        ...
                        <div class="flex items-center justify-between gap-3 rounded-xl border border-mist-100 px-3 py-2.5 dark:border-ink-700">
                            <div class="min-w-0">
                            ...
                            <div class="shrink-0">
                                @if ($today?->check_in === null)
                                    <form method="POST" action="{{ route('hr.attendance.check-in') }}">
                                        ...
                                        <button type="submit" class="rounded-lg bg-emerald-400 px-2.5 py-1 text-xs font-semibold text-emerald-900">حضور</button>
                                    </form>
                    ...
                    @forelse ($logs as $log)
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-4 py-3 text-start">
                                <a href="{{ route('hr.employees.show', ['employee' => $log->employee, 'tab' => 'attendance']) }}" class="font-medium text-ink-900 hover:text-emerald-600 dark:text-ink-50">
                                    {{ $log->employee?->full_name ?? '—' }}
                                </a>
                            </td>
                            <td class="px-4 py-3 text-mist-500 text-start">{{ $log->employee?->department?->name ?? '—' }}</td>
                            <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $log->check_in?->format('H:i') ?? '—' }}</x-ui.ltr></td>
                            <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $log->check_out?->format('H:i') ?? '—' }}</x-ui.ltr></td>
                            <td class="px-4 py-3 tabular-nums text-mist-600 text-start"><x-ui.ltr>{{ $log->workedHoursLabel() }}</x-ui.ltr></td>
                            <td class="px-4 py-3 text-center">
                                <span @class(['inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold', $statusClasses[$log->status->value] ?? ''])>
                                    {{ $log->status->label() }}
                                </span>
                            </td>
                        </tr>
                    @empty
                        <x-ui.table-empty :colspan="7" icon="📋" message="لا توجد سجلات لهذا التاريخ." />
                    @endforelse
                </tbody>
            </table>
        </div>
