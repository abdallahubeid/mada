@php
    ...
    $primaryBtn = 'inline-flex items-center justify-center gap-2 rounded-full bg-emerald-500 px-6 py-3 text-sm font-semibold text-ink-950 shadow-glow transition duration-200 ease-in-out hover:bg-emerald-400 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50';
...
<x-layouts.guest max-width="max-w-3xl" title="إعداد المؤسسة — Veyra ERP">
    <div class="mb-8 text-center">
        ...
        <h1 class="mt-6 font-display text-2xl font-bold text-ink-900 dark:text-ink-50 sm:text-3xl">إعداد مؤسستك</h1>
    ...
    >
                ...
                <div class="flex items-center {{ $index < count($steps) - 1 ? 'flex-1' : '' }}">
                    <div class="flex flex-col items-center gap-2">
                        ...
                        <span
                            ...
                            class="hidden text-[11px] font-medium sm:block"
                        >
                            {{ $label }}
                        </span>
                    </div>
        ...
        <form method="POST" action="{{ route('dashboard.setup.update') }}" enctype="multipart/form-data" class="space-y-6" novalidate>
            ...
            <div x-show="step === 1" x-cloak class="space-y-4">
                <div>
                    <h2 class="font-display text-lg font-semibold text-ink-900 dark:text-ink-50">تحديث كلمة المرور</h2>
            ...
            <div x-show="step === 2" x-cloak class="space-y-4">
                <div>
                    <h2 class="font-display text-lg font-semibold text-ink-900 dark:text-ink-50">شعار الشركة</h2>
                ...
                <div>
                    ...
                    <input id="logo" name="logo" type="file" accept="image/*" class="{{ $inputClasses }} file:me-3 file:rounded-full file:border-0 file:bg-emerald-500/15 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-emerald-700 dark:file:text-emerald-300">
                    @error('logo')
                        <p class="{{ $errorClasses }}">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div x-show="step === 3" x-cloak class="space-y-4">
                <div>
                    <h2 class="font-display text-lg font-semibold text-ink-900 dark:text-ink-50">العملة والمنطقة الزمنية</h2>
            ...
            <div x-show="step === 4" x-cloak class="space-y-4">
                <div>
                    <h2 class="font-display text-lg font-semibold text-ink-900 dark:text-ink-50">تقويم العمل</h2>
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <button type="button" @click="addHoliday()" class="text-sm font-medium text-emerald-600 hover:text-emerald-500 dark:text-emerald-400">
                            إضافة عطلة
                        </button>
                    </div>
                    ...
                    <template x-for="(holiday, index) in holidays" :key="index">
                        <div class="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                            ...
                            <button type="button" @click="removeHoliday(index)" class="{{ $secondaryBtn }} px-4 py-2.5">حذف</button>
                        </div>
                    </template>
                    @error('holidays')
                        <p class="{{ $errorClasses }}">{{ $message }}</p>
        ...
        <div class="mt-6 border-t border-mist-200 pt-4 text-center dark:border-ink-600">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-sm text-mist-500 underline-offset-2 hover:text-emerald-600 hover:underline dark:text-mist-400 dark:hover:text-emerald-400">
                    تسجيل الخروج
                </button>
            </form>
        </div>
