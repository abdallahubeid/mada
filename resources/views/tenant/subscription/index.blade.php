@php
    ...
    $statusClasses = [
        SubscriptionStatus::Active->value => 'bg-brand-500/15 text-brand-700 dark:text-brand-300',
    ...
    $invoiceStatusClasses = [
        TenantInvoiceStatus::Paid->value => 'bg-brand-500/15 text-brand-700 dark:text-brand-300',
...
<x-layouts.app title="إدارة الاشتراك والخطط">
    <div class="mx-auto max-w-5xl space-y-6">
        <div>
            <h1 class="font-display text-2xl font-medium text-ink-900 dark:text-ink-50">إدارة الاشتراك والخطط</h1>
            <p class="mt-1 text-sm text-mist-500">راجع خطتك الحالية، استخدام الموارد، وسجل الفواتير.</p>
        </div>

        {{-- Header card --}}
        <section class="rounded-2xl border border-mist-200 bg-gradient-to-br from-brand-500/15 via-white to-sky-400/10 p-4 shadow-sm dark:border-ink-600 dark:from-brand-500/10 dark:via-ink-800 dark:to-ink-800 sm:p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    <div class="flex flex-wrap items-center gap-2">
                        <h2 class="font-display text-xl font-medium text-ink-900 dark:text-ink-50">{{ $planName }}</h2>
                        <span @class(['inline-flex rounded-md px-2.5 py-0.5 text-xs font-semibold', $statusClasses[$status->value] ?? ''])>
                            {{ $status->label() }}
                        </span>
                        @if ($renewalWarning)
                            <span class="inline-flex rounded-md bg-amber-400/15 px-2.5 py-0.5 text-xs font-semibold text-amber-800 dark:text-amber-300">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a
                        ...
                        class="rounded-xl border border-mist-200 bg-white/70 px-4 py-2 text-sm font-semibold text-ink-700 transition hover:border-brand-500 hover:text-brand-700 dark:border-ink-600 dark:bg-ink-900/50 dark:text-mist-200"
                    >
                        تجديد الاشتراك
                    </a>
                </div>
            ...
            @foreach ($usage as $meter)
                <div class="rounded-2xl border border-mist-200 bg-white p-4 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                    ...
                    <div class="mt-3 h-2 overflow-hidden rounded-full bg-mist-100 dark:bg-ink-700">
                        <div
                            class="h-full rounded-full transition-all {{ $meter['percent'] >= 90 ? 'bg-amber-400' : 'bg-brand-500' }}"
                ...
                <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                    <thead class="bg-mist-50 dark:bg-ink-900">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-xs font-medium text-mist-500 dark:text-mist-400 text-start">رقم الفاتورة</th>
                            <th class="px-3 py-2 text-xs font-medium text-mist-500 dark:text-mist-400 text-start">المبلغ</th>
                            <th class="px-3 py-2 text-xs font-medium text-mist-500 dark:text-mist-400 text-center">الحالة</th>
                            <th class="px-3 py-2 text-xs font-medium text-mist-500 dark:text-mist-400 text-end">PDF</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                        @forelse ($invoices as $invoice)
                            <tr>
                                ...
                                <td class="px-3 py-2 tabular-nums text-start"><x-ui.ltr>{{ $invoice->issued_at?->format('Y-m-d') }}</x-ui.ltr></td>
                                <td class="px-3 py-2 font-medium text-start"><x-ui.ltr>{{ $invoice->number }}</x-ui.ltr></td>
                                <td class="px-3 py-2 tabular-nums text-start"><x-ui.ltr>{{ rtrim(rtrim(number_format((float) $invoice->amount, 2, '.', ''), '0'), '.') }}
                                    {{ $invoice->currency }}</x-ui.ltr></td>
                                <td class="px-3 py-2 text-center">
                                    <span @class(['inline-flex rounded-md px-2.5 py-0.5 text-xs font-semibold', $invoiceStatusClasses[$invoice->status->value] ?? ''])>
                        ...
                        @empty
                            <x-ui.table-empty :colspan="6" icon="card" message="لا توجد فواتير بعد." />
                        @endforelse
                    </tbody>
                </table>
            </div>
