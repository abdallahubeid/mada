@php
    ...
    $kindClasses = [
        PayslipLineItemKind::Allowance->value => 'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300',
...
<x-layouts.app title="بنود البدلات والاستقطاعات">
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">بنود البدلات والاستقطاعات</h1>
            ...
            <a href="{{ route('finance.line-item-types.create') }}" class="inline-flex items-center justify-center rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300">
                إضافة بند
            </a>
        </div>
            ...
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                <thead class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-end text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400">القيمة الافتراضية</th>
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    @forelse ($types as $type)
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-4 py-3 text-start font-medium text-ink-900 dark:text-ink-50">{{ $type->name }}</td>
                            <td class="px-4 py-3 text-start text-mist-500"><x-ui.ltr>{{ $type->code ?? '—' }}</x-ui.ltr></td>
                            <td class="px-4 py-3 text-center">
                                <span @class(['inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold', $kindClasses[$type->kind->value] ?? ''])>
                                    {{ $type->kind->label() }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-end text-ink-700 dark:text-mist-200">
                                <x-ui.money :amount="$type->default_amount" :signed="true" />
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span @class([
                                    'inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold',
                                    'bg-emerald-400/15 text-emerald-700 dark:text-emerald-300' => $type->is_active,
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('finance.line-item-types.edit', $type) }}" class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600">تعديل</a>
                    ...
                    @empty
                        <x-ui.table-empty :colspan="7" icon="🧾" message="لا توجد بنود بعد." hint="أضف بدلاً أو استقطاعاً ليُطبَّق تلقائياً على المسيرات الجديدة." />
                    @endforelse
                </tbody>
            </table>
        </div>
