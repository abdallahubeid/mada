<x-layouts.app title="تصنيفات المصروفات">
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-medium text-ink-900 dark:text-ink-50">تصنيفات المصروفات</h1>
                <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">تُستخدم لتحليل التكاليف في لوحة التحكم المالية.</p>
            </div>
            <a href="{{ route('finance.expense-categories.create') }}" class="inline-flex items-center justify-center rounded-xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white shadow-glow transition hover:bg-brand-600">إضافة تصنيف</a>
        </div>
            ...
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                <thead class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-center text-xs font-medium text-mist-500 dark:text-mist-400">عدد المصروفات</th>
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    @forelse ($categories as $category)
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-3 py-2 text-start font-medium text-ink-900 dark:text-ink-50">{{ $category->name }}</td>
                            <td class="px-3 py-2 text-start text-mist-500"><x-ui.ltr>{{ $category->code ?? '—' }}</x-ui.ltr></td>
                            <td class="px-3 py-2 text-center text-mist-500"><x-ui.ltr>{{ $category->expenses_count }}</x-ui.ltr></td>
                            <td class="px-3 py-2 text-center">
                                <span @class([
                                    'inline-flex rounded-md px-2.5 py-0.5 text-xs font-semibold',
                                    'bg-brand-500/15 text-brand-700 dark:text-brand-300' => $category->is_active,
                            ...
                            <td class="px-3 py-2 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('finance.expense-categories.edit', $category) }}" class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold transition hover:border-brand-500 hover:text-brand-600 dark:border-ink-600">تعديل</a>
                    ...
                    @empty
                        <x-ui.table-empty :colspan="6" icon="folder" message="لا توجد تصنيفات بعد." />
                    @endforelse
                </tbody>
            </table>
        </div>
