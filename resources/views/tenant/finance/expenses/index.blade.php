...
<x-layouts.app title="المصروفات">
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">المصروفات</h1>
            ...
            <div class="flex flex-wrap gap-2">
                @can('finance.expense_categories.manage')
                    <a href="{{ route('finance.expense-categories.index') }}" class="rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600">التصنيفات</a>
                @endcan
                @can('finance.expenses.manage')
                    <a href="{{ route('finance.expenses.create') }}" class="inline-flex items-center justify-center rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300">تسجيل مصروف</a>
                @endcan
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                <thead class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400">البيان</th>
                        ...
                        <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-mist-500 dark:text-mist-400">مطالبة</th>
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    @forelse ($expenses as $expense)
                        <tr class="transition hover:bg-mist-50/80 dark:hover:bg-ink-900/40">
                            ...
                            <td class="px-4 py-3 text-start font-medium text-ink-900 dark:text-ink-50">{{ $expense->title }}</td>
                            <td class="px-4 py-3 text-start text-mist-500">{{ $expense->category?->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-start text-mist-500"><x-ui.ltr>{{ $expense->expense_date?->format('Y-m-d') }}</x-ui.ltr></td>
                            <td class="px-4 py-3 text-center text-mist-500">{{ $expense->is_claimable ? 'نعم' : 'لا' }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('finance.expenses.show', $expense) }}" class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600">عرض</a>
                                    @can('finance.expenses.manage')
                                        @if ($expense->status->isEditable())
                                            <a href="{{ route('finance.expenses.edit', $expense) }}" class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold transition hover:border-emerald-400 hover:text-emerald-600 dark:border-ink-600">تعديل</a>
                    ...
                    @empty
                        <x-ui.table-empty :colspan="8" icon="🧾" message="لا توجد مصروفات بعد." hint="سجّل أول مصروف ليبدأ مسار الاعتماد." />
                    @endforelse
                </tbody>
            </table>
        </div>
