...
<x-layouts.app title="قسيمة راتب {{ $payslip->employee_name }}">
    <div class="mx-auto max-w-4xl space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h1 class="font-display text-2xl font-medium text-ink-900 dark:text-ink-50">{{ $payslip->employee_name }}</h1>
            ...
            <a href="{{ route('finance.payslips.print', $payslip) }}" target="_blank" rel="noopener" class="rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold transition hover:border-brand-500 hover:text-brand-600 dark:border-ink-600">
                نسخة للطباعة
            </a>
        </div>
        ...
        <div class="w-full overflow-x-auto rounded-2xl border border-mist-200 bg-white shadow-sm dark:border-ink-600 dark:bg-ink-800">
            <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
                ...
                <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                    <tr>
                        <td class="w-12 px-3 py-2 text-center text-sm tabular-nums text-mist-500">1</td>
                        <td class="px-3 py-2 text-start font-medium text-ink-900 dark:text-ink-50">الراتب الأساسي</td>
                        <td class="px-3 py-2 text-center text-mist-500">أساسي</td>
                        <td class="px-3 py-2 text-end text-ink-700 dark:text-mist-200"><x-ui.money :amount="$payslip->base_amount" /></td>
                    </tr>
                    @if ($payslip->absence_deduction !== 0)
                        <tr>
                            <td class="w-12 px-3 py-2 text-center text-sm tabular-nums text-mist-500">2</td>
                            ...
                            <td class="px-3 py-2 text-center text-mist-500">استقطاع</td>
                            <td class="px-3 py-2 text-end text-danger-solid"><x-ui.money :amount="$payslip->absence_deduction" :signed="true" /></td>
                        </tr>
                    @endif
                    @forelse ($payslip->lineItems as $lineItem)
                        <tr>
                            ...
                            <td @class([
                                'px-4 py-3 text-end',
                                'text-brand-600 dark:text-brand-300' => $lineItem->kind === PayslipLineItemKind::Allowance,
                ...
                <tfoot class="bg-mist-50 dark:bg-ink-900">
                    <tr>
                        <td colspan="3" class="px-3 py-2 text-end text-sm font-semibold text-ink-700 dark:text-mist-200">الإجمالي</td>
                        <td class="px-3 py-2 text-end text-sm font-semibold text-ink-700 dark:text-mist-200">
                            <x-ui.money :amount="$payslip->gross_amount" />
                        </td>
                    </tr>
                    <tr>
                        <td colspan="3" class="px-3 py-2 text-end font-display text-base font-medium text-ink-900 dark:text-ink-50">صافي المستحق</td>
        ...
        @can('finance.payroll.view_any')
            <a href="{{ route('finance.payroll-runs.show', $payslip->payroll_run_id) }}" class="inline-block text-sm font-semibold text-brand-600 hover:underline dark:text-brand-300">
                ← العودة إلى المسيرة
            </a>
        @endcan
    </div>
