...
<head>
    ...
    <style>
        .portal-grid-bg {
            background-image:
                radial-gradient(ellipse 80% 50% at 50% -20%, rgba(113, 75, 103, 0.22), transparent 55%),
                radial-gradient(ellipse 60% 40% at 100% 0%, rgba(113, 75, 103, 0.08), transparent 45%),
        ...
        .dark .portal-grid-bg {
            background-image:
                radial-gradient(ellipse 80% 50% at 50% -20%, rgba(113, 75, 103, 0.14), transparent 55%),
                radial-gradient(ellipse 50% 35% at 90% 10%, rgba(113, 75, 103, 0.06), transparent 45%),
    ...
    <header class="sticky top-0 z-50 border-b border-mist-200/60 bg-white/80 backdrop-blur-xl dark:border-ink-700/60 dark:bg-ink-950/75">
        <div class="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6">
            <a href="{{ route('portal.index', $slug) }}" class="flex min-w-0 items-center gap-3">
                <span class="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-500 font-display text-lg font-medium text-white shadow-lg">
                    {{ $company['logo_initial'] }}
                </span>
                <span class="min-w-0">
                    <span class="block truncate font-display text-sm font-medium text-ink-900 dark:text-ink-50">{{ $company['name'] }}</span>
                    <span class="block text-xs text-mist-500">بوابة التوظيف</span>
                </span>
            </a>
                ...
                @foreach ($nav as $item)
                    <a
                        ...
                        @class([
                            ...
                            'bg-brand-500/10 text-brand-600 dark:text-brand-300' => $item['active'],
            ...
            <div class="flex items-center gap-2">
                @if ($portalSettings->isSectionActive('careers'))
                    <a
                        ...
                        class="hidden rounded-xl bg-brand-500 px-4 py-2 text-sm font-bold text-white shadow-lg transition hover:bg-brand-600 sm:inline-flex"
                    >
                        الشواغر المتاحة
                    </a>
                @endif
        ...
        <div x-show="navOpen" x-cloak x-transition class="border-t border-mist-200 bg-white px-4 py-3 lg:hidden dark:border-ink-700 dark:bg-ink-900">
            <div class="flex flex-col gap-1">
                @foreach ($nav as $item)
                    <a href="{{ $item['href'] }}" @click="navOpen = false" class="rounded-lg px-3 py-2 text-sm font-medium text-ink-700 hover:bg-mist-100 dark:text-mist-200 dark:hover:bg-ink-800">{{ $item['label'] }}</a>
                @endforeach
                @if ($portalSettings->isSectionActive('careers'))
                    <a href="{{ route('portal.careers', $slug) }}" class="mt-2 rounded-xl bg-brand-500 px-3 py-2 text-center text-sm font-bold text-white">الشواغر المتاحة</a>
                @endif
            </div>
        </div>
    </header>
    ...
    <footer class="relative mt-20 overflow-hidden border-t border-mist-200 bg-ink-950 text-mist-300">
        ...
        <div class="relative mx-auto grid max-w-6xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-3">
            <div>
                <div class="flex items-center gap-3">
                    <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500 font-display text-lg font-medium text-white">{{ $company['logo_initial'] }}</span>
                    <div>
                        <p class="font-display text-sm font-medium text-white">{{ $company['name'] }}</p>
            ...
            <div>
                <p class="text-xs font-semibold text-mist-500 uppercase">التنقّل</p>
                <ul class="mt-3 space-y-2 text-sm">
                    <li><a href="{{ route('portal.index', $slug) }}" class="transition hover:text-brand-300">الرئيسية</a></li>
                    <li><a href="{{ route('portal.careers', $slug) }}" class="transition hover:text-brand-300">الوظائف</a></li>
                    <li><a href="{{ route('portal.contact', $slug) }}" class="transition hover:text-brand-300">تواصل معنا</a></li>
                </ul>
            </div>
            <div>
                <p class="text-xs font-semibold text-mist-500 uppercase">التواصل</p>
                <ul class="mt-3 space-y-2 text-sm">
        ...
        <div class="relative border-t border-white/10">
            <div class="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-4 text-xs text-mist-500 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                ...
                <p>مدعوم بواسطة <span class="text-brand-300">مدى</span></p>
            </div>
        </div>
    </footer>
