...
@section('content')
    ...
    <section class="relative overflow-hidden border-b border-mist-200 dark:border-ink-700">
        ...
        <div class="relative mx-auto max-w-6xl px-4 py-10 sm:px-6">
            <a href="{{ route('portal.careers', $slug) }}" class="text-sm font-bold text-emerald-600 hover:text-emerald-500 dark:text-emerald-400">← العودة إلى الوظائف</a>
            <div class="mt-4">
                ...
                <h1 class="mt-3 font-display text-3xl font-bold text-ink-900 sm:text-4xl dark:text-ink-50">{{ $job['title'] }}</h1>
    ...
    <section class="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div class="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <article class="rounded-2xl border border-mist-200 bg-white p-6 shadow-sm dark:border-ink-600 dark:bg-ink-800 sm:p-8">
                <h2 class="font-display text-lg font-semibold text-ink-900 dark:text-ink-50">نبذة عن الدور</h2>
                ...
                @if (! empty($job['responsibilities']))
                    <h2 class="mt-8 font-display text-lg font-semibold text-ink-900 dark:text-ink-50">المتطلبات / المسؤوليات</h2>
                    <ul class="mt-4 space-y-3">
                        @foreach ($job['responsibilities'] as $item)
                            <li class="flex gap-3 rounded-xl border border-mist-100 bg-mist-50/70 px-3 py-2.5 text-sm text-mist-600 dark:border-ink-700 dark:bg-ink-900/50 dark:text-mist-300">
                                <span class="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-400 text-[10px] font-bold text-emerald-950">✓</span>
                                <span>{{ $item }}</span>
                            </li>
                        @endforeach
                    </ul>
            ...
            <aside class="h-fit rounded-2xl border border-emerald-400/20 bg-gradient-to-b from-white to-emerald-50/40 p-6 shadow-[0_0_40px_rgba(78,222,163,0.12)] dark:border-emerald-400/20 dark:from-ink-800 dark:to-ink-900 sm:p-7">
                <h2 class="font-display text-lg font-semibold text-ink-900 dark:text-ink-50">قدّم على هذه الوظيفة</h2>
                ...
                >
                    ...
                    <div>
                        ...
                        <input id="cv" name="cv" type="file" required accept=".pdf,.doc,.docx" class="{{ $inputClass }} file:me-3 file:rounded-full file:border-0 file:bg-emerald-500/15 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-emerald-700 dark:file:text-emerald-300">
                        @error('cv')
                            <p class="{{ $errorClass }}">{{ $message }}</p>
                        @enderror
                    </div>
                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-xl bg-emerald-400 px-4 py-3 text-sm font-bold text-emerald-950 shadow-[0_0_28px_rgba(78,222,163,0.35)] transition hover:bg-emerald-300">
                        إرسال الطلب
                    </button>
                </form>
            </aside>
