...
>
    ...
    <main>
        <x-marketing.page-hero
            ...
            subtitle="صُمّم مدى من الأساس لعزل البيانات والحوكمة والتدقيق — خاصة للجهات الحكومية والتعليمية وغير الربحية."
        />

        <section class="bg-white py-24 sm:py-24 dark:bg-ink-900">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <x-marketing.section-heading
                    title="ركائز الأمان في مدى"
                    subtitle="ضمانات تقنية وتشغيلية يمكنك الاعتماد عليها عند تقييم المنصّة."
                />

                <div class="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($pillars as $pillar)
                        <div class="group rounded-2xl border border-mist-200 bg-ink-50/40 p-6 transition duration-200 ease-out hover:-translate-y-1 hover:border-brand-500/50 hover:shadow-lg dark:border-ink-800 dark:bg-ink-800/60">
                            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-500/10 text-brand-600 transition duration-200 group-hover:bg-brand-600 group-hover:text-white dark:text-brand-300">
                            ...
                            <h3 class="mt-5 font-display text-lg font-medium text-ink-900 dark:text-ink-50">{{ $pillar['title'] }}</h3>
        ...
        {{-- Lifecycle callout --}}
        <section class="bg-ink-100 py-24 sm:py-24 dark:bg-ink-950">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <x-marketing.section-heading
                ...
                <ol class="mx-auto mt-14 grid max-w-5xl gap-4 sm:grid-cols-5">
                    @foreach ([
                        ...
                        <li class="relative rounded-2xl border border-mist-200 bg-white p-4 text-center dark:border-ink-800 dark:bg-ink-800/60">
                            <span class="mx-auto flex h-8 w-8 items-center justify-center rounded-full bg-brand-500/15 font-display text-sm font-medium text-brand-700 dark:text-brand-300">{{ $i + 1 }}</span>
