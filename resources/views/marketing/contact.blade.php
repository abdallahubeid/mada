...
<x-layouts.marketing
    title="تواصل معنا — مدى"
    description="تواصل مع فريق مدى لحجز عرض توضيحي، استفسارات المبيعات، أو الدعم. نرد عادة خلال يوم عمل واحد."
>
    <x-marketing.nav />
    ...
    <main>
        ...
        <section class="bg-white py-24 sm:py-24 dark:bg-ink-900">
            <div class="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-5 lg:gap-16 lg:px-8">
                ...
                <aside class="space-y-6 lg:col-span-2">
                    <div class="rounded-2xl border border-mist-200 bg-ink-50/40 p-6 dark:border-ink-800 dark:bg-ink-800/60">
                        <h2 class="font-display text-lg font-medium text-ink-900 dark:text-ink-50">معلومات التواصل</h2>
                        <ul class="mt-4 space-y-4 text-sm text-mist-600 dark:text-mist-300">
                            <li class="flex items-start gap-3">
                                <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-500/10 text-brand-600 dark:text-brand-300">
                                ...
                                <div>
                                    ...
                                    <a href="mailto:hello@mada.app" class="text-brand-600 hover:underline dark:text-brand-300">hello@mada.app</a>
                                </div>
                            </li>
                            <li class="flex items-start gap-3">
                                <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-500/10 text-brand-600 dark:text-brand-300">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            <div>
                                ...
                                <input id="name" name="name" type="text" value="{{ old('name') }}" required autocomplete="name"
                                    class="w-full rounded-xl border border-mist-200 bg-white px-3 py-2 text-sm text-ink-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50 @error('name') border-danger-solid @enderror">
                            ...
                            <div>
                                ...
                                <input id="email" name="email" type="email" value="{{ old('email') }}" required autocomplete="email"
                                    class="w-full rounded-xl border border-mist-200 bg-white px-3 py-2 text-sm text-ink-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50 @error('email') border-danger-solid @enderror">
                            ...
                            <div>
                                ...
                                <input id="company" name="company" type="text" value="{{ old('company') }}" autocomplete="organization"
                                    class="w-full rounded-xl border border-mist-200 bg-white px-3 py-2 text-sm text-ink-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50">
                            </div>

                            <div>
                                <label for="subject" class="mb-1.5 block text-sm font-medium text-ink-700 dark:text-mist-200">الموضوع</label>
                                <select id="subject" name="subject" required
                                    class="w-full rounded-xl border border-mist-200 bg-white px-3 py-2 text-sm text-ink-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50 @error('subject') border-danger-solid @enderror">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea id="message" name="message" rows="6" required
                                    class="w-full rounded-xl border border-mist-200 bg-white px-3 py-2 text-sm text-ink-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50 @error('message') border-danger-solid @enderror">{{ old('message') }}</textarea>
                                @error('message') <p class="mt-1.5 text-xs text-danger-solid">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div class="mt-6 flex justify-end">
                            <button type="submit" class="inline-flex items-center gap-2 rounded-md bg-brand-500 px-6 py-3 text-sm font-semibold text-white shadow-glow transition duration-200 hover:bg-brand-600 active:scale-[0.98]">
                                إرسال الرسالة
