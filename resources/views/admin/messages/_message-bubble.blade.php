...
<div class="flex items-end gap-2 {{ $isAdmin ? 'flex-row-reverse' : 'flex-row' }}" data-message-id="{{ $message['id'] }}">
    ...
    <div class="max-w-[80%]">
        <div @class([
            ...
            'bg-brand-500 text-white rounded-se-none' => $isAdmin,
        ...
        <div class="mt-1 flex items-center gap-1.5 px-1 text-xs text-mist-400 dark:text-mist-500 {{ $isAdmin ? 'justify-end' : 'justify-start' }}">
            <span>
                {{ $message['sender_name'] }} ·
