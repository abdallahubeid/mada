...
@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">إدارة صلاحيات أدوار منصّة مدى حسب النطاق.</p>
        </div>
        @can('roles.create')
            <a href="{{ route('admin.roles.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white shadow-glow transition hover:bg-brand-600">إنشاء دور جديد</a>
        @endcan
    </div>
        ...
        <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
            <thead class="bg-mist-50 text-mist-500 dark:bg-ink-900 dark:text-mist-400">
                <tr>
                    ...
                    <th class="px-3 py-2 text-start font-medium">المعرّف</th>
                    <th class="px-3 py-2 text-start font-medium">عدد الصلاحيات</th>
                    <th class="px-3 py-2 text-start font-medium">عدد المستخدمين</th>
            ...
            <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                @forelse ($roles as $role)
                    <tr>
                        ...
                        <td class="px-3 py-2 font-medium text-ink-900 dark:text-ink-50">{{ $roleLabels[$role->name] ?? $role->name }}</td>
                        <td class="px-3 py-2 font-mono text-xs text-mist-500">{{ $role->name }}</td>
                        <td class="px-3 py-2 text-mist-500">{{ $role->permissions_count }}</td>
                        <td class="px-3 py-2 text-mist-500">{{ $role->users_count }}</td>
                        <td class="px-3 py-2">
