...
@section('content')
    @php
        $actionMeta = [
            'approval' => ['label' => 'موافقة', 'badge' => 'bg-brand-500/10 text-brand-600 dark:text-brand-300', 'dot' => 'bg-brand-500'],
        ...
        $selectClass = 'rounded-xl border border-mist-200 bg-white px-3 py-2 text-sm text-ink-700 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-ink-600 dark:bg-ink-800 dark:text-ink-50';
    @endphp
    ...
    <div x-data="{ open: false, entry: null }">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-display text-2xl font-medium text-ink-900 dark:text-ink-50">سجل النشاط والرقابة</h2>
            ...
            <button type="button" disabled title="يتوفّر في المرحلة 4" class="inline-flex cursor-not-allowed items-center gap-2 rounded-xl border border-mist-200 px-4 py-2 text-sm font-medium text-mist-400 dark:border-ink-600 dark:text-mist-500">
                ...
                تصدير
                <span class="rounded-md bg-mist-100 px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-mist-500 dark:bg-ink-700 dark:text-mist-400">Phase 4</span>
            </button>
        </div>
        ...
        <form method="GET" class="mt-6 grid grid-cols-1 gap-3 rounded-2xl border border-mist-200 bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-4 dark:border-ink-600 dark:bg-ink-800">
            ...
            <div>
                ...
                <div class="flex gap-2">
                    ...
                    <button type="submit" class="shrink-0 rounded-xl bg-brand-500 px-3 py-2 text-sm font-semibold text-white shadow-glow transition duration-200 hover:bg-brand-600 active:scale-95">تصفية</button>
                </div>
            </div>
        </form>
                ...
                <table class="w-full min-w-max text-start text-sm">
                    <thead>
                        <tr class="border-b border-mist-100 text-xs uppercase tracking-wide text-mist-500 dark:border-ink-700 dark:text-mist-400">
                            <th class="px-3 py-2 text-start font-medium">التوقيت</th>
                            <th class="px-3 py-2 text-start font-medium">المنفّذ</th>
                            <th class="px-3 py-2 text-start font-medium">نوع الإجراء</th>
                            <th class="px-3 py-2 text-start font-medium">الهدف</th>
                            <th class="px-3 py-2 text-start font-medium">عنوان IP</th>
                            <th class="px-3 py-2 text-end font-medium">التفاصيل</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="transition duration-150 hover:bg-mist-50 dark:hover:bg-ink-700/40">
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center gap-2">
                                        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-xs font-bold {{ $entry['actor_type'] === 'system' ? 'bg-sky-500/15 text-sky-500' : 'bg-brand-500/15 text-brand-600 dark:text-brand-300' }}">
                                ...
                                <td class="px-3 py-2">
                                    <span class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium {{ $actionMeta[$entry['action']]['badge'] }}">
                                        <span class="h-1.5 w-1.5 rounded-full {{ $actionMeta[$entry['action']]['dot'] }}"></span>
                                        {{ $actionMeta[$entry['action']]['label'] }}
                                    </span>
                                </td>
                                <td class="px-3 py-2 text-ink-700 dark:text-mist-200">{{ $entry['target'] }}</td>
                                <td class="px-3 py-2 font-mono text-xs text-mist-500 dark:text-mist-400">{{ $entry['ip'] }}</td>
        ...
        >
            <div class="flex items-center justify-between border-b border-mist-100 px-5 py-4 dark:border-ink-700">
                <h3 class="font-display text-base font-medium text-ink-900 dark:text-ink-50">تفاصيل السجل</h3>
            ...
            <div class="flex-1 space-y-5 overflow-y-auto p-4" x-show="entry">
                ...
                {{-- Field-level diff --}}
                <div>
                    <p class="mb-2 text-xs font-semibold uppercase text-mist-500 dark:text-mist-400">التغييرات</p>
                    <div class="space-y-2">
                        <template x-for="(change, i) in (entry?.changes || [])" :key="i">
                            <div class="rounded-xl border border-mist-200 p-3 dark:border-ink-600">
                                ...
                                <div class="mt-1.5 flex items-center gap-2 text-sm">
                                    ...
                                    <span class="rounded-md bg-brand-500/10 px-2 py-0.5 font-mono text-xs text-brand-600 dark:text-brand-300" x-text="change.to"></span>
                                </div>
                            </div>
                        </template>
                    </div>
                ...
                {{-- Raw JSON payload --}}
                <div>
                    <p class="mb-2 text-xs font-semibold uppercase text-mist-500 dark:text-mist-400">الحمولة الكاملة (JSON)</p>
