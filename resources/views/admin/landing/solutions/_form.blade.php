@php
    ...
    $image = $solution->relationLoaded('images')
        ? $solution->images->firstWhere('collection', 'icon')
        : $solution->image('icon')->first();
@endphp
...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-4 rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
    ...
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $solution->title) }}" class="{{ $inputClass }}" required>
    </div>

    <div>
        <label class="{{ $labelClass }}">الوصف</label>
        <textarea name="description" rows="4" class="{{ $inputClass }}" required>{{ old('description', $solution->description) }}</textarea>
    </div>

    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
            <label class="{{ $labelClass }}">نص الزر</label>
            <input type="text" name="btn_text" value="{{ old('btn_text', $solution->btn_text) }}" class="{{ $inputClass }}">
        </div>
        <div>
            <label class="{{ $labelClass }}">رابط الزر</label>
            <input type="text" dir="ltr" name="btn_link" value="{{ old('btn_link', $solution->btn_link) }}" class="{{ $inputClass }}">
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
            ...
            <input type="text" dir="ltr" name="icon_key" value="{{ old('icon_key', $solution->icon_key) }}" class="{{ $inputClass }}" placeholder="shield">
        </div>
        <div>
            <label class="{{ $labelClass }}">ترتيب العرض</label>
            <input type="number" name="sort_order" value="{{ old('sort_order', $solution->sort_order) }}" class="{{ $inputClass }}">
        </div>
    </div>

    <label class="flex items-center gap-2 text-sm text-ink-700 dark:text-mist-200">
        <input type="checkbox" name="is_published" value="1" @checked(old('is_published', $solution->is_published)) class="rounded border-mist-300 text-emerald-500 focus:ring-emerald-400">
        منشور على الموقع
    </label>
    ...
    <div class="flex justify-end gap-3 pt-2">
        <a href="{{ route('admin.solutions.index') }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-mist-600">إلغاء</a>
