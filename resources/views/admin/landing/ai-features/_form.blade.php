@php
    ...
    $image = $aiFeature->relationLoaded('images')
        ? $aiFeature->images->firstWhere('collection', 'icon')
        : $aiFeature->image('icon')->first();
@endphp
...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-4 rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
    ...
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $aiFeature->title) }}" class="{{ $inputClass }}" required>
    </div>

    <div>
        <label class="{{ $labelClass }}">الوصف</label>
        <textarea name="description" rows="4" class="{{ $inputClass }}" required>{{ old('description', $aiFeature->description) }}</textarea>
    </div>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
            ...
            <input type="text" dir="ltr" name="icon_key" value="{{ old('icon_key', $aiFeature->icon_key) }}" class="{{ $inputClass }}" placeholder="shield">
        </div>
        <div>
            <label class="{{ $labelClass }}">ترتيب العرض</label>
            <input type="number" name="sort_order" value="{{ old('sort_order', $aiFeature->sort_order) }}" class="{{ $inputClass }}">
        </div>
    </div>

    <label class="flex items-center gap-2 text-sm text-ink-700 dark:text-mist-200">
        <input type="checkbox" name="is_published" value="1" @checked(old('is_published', $aiFeature->is_published)) class="rounded border-mist-300 text-emerald-500 focus:ring-emerald-400">
        منشور على الموقع
    </label>
    ...
    <div class="flex justify-end gap-3 pt-2">
        <a href="{{ route('admin.ai-features.index') }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-mist-600">إلغاء</a>
