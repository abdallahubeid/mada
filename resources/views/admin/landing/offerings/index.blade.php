...
@section('breadcrumbs')
    ...
    <span class="text-ink-700 dark:text-mist-200">ما نقدمه</span>
@endsection

@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">ما نقدمه</h2>
            <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">إدارة عناصر قسم ما نقدمه في صفحة الهبوط.</p>
        </div>
        <a href="{{ route('admin.offerings.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300">إضافة عرض</a>
    </div>
        ...
        <table class="min-w-full divide-y divide-mist-100 text-sm dark:divide-ink-700">
            ...
            <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                @forelse ($offerings as $offering)
                    @php
                        $image = $offering->images->firstWhere('collection', 'icon');
                    @endphp
                    <tr>
                        <td class="px-4 py-3">
                            @if ($image)
                        ...
                        <td class="px-4 py-3 text-mist-500">{{ $offering->sort_order }}</td>
                        <td class="px-4 py-3 font-medium text-ink-900 dark:text-ink-50">{{ $offering->title }}</td>
                        <td class="px-4 py-3 text-mist-500">{{ $offering->icon }}</td>
                        <td class="px-4 py-3">
                            <span @class([
                                'rounded-full px-2 py-0.5 text-[10px] font-semibold',
                                'bg-emerald-500/10 text-emerald-600' => $offering->is_published,
                                'bg-mist-100 text-mist-500' => ! $offering->is_published,
                            ])>{{ $offering->is_published ? 'منشور' : 'مسودة' }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.offerings.edit', $offering) }}" class="rounded-lg border border-mist-200 px-3 py-1.5 text-xs font-semibold dark:border-ink-600">تعديل</a>
                                <form method="POST" action="{{ route('admin.offerings.destroy', $offering) }}" data-swal-confirm data-swal-title="حذف هذا العنصر؟">
                                    @csrf
                                    @method('DELETE')
