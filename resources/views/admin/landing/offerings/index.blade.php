...
@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-medium text-ink-900 dark:text-ink-50">ما نقدمه</h2>
            <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">إدارة عناصر قسم ما نقدمه في صفحة الهبوط.</p>
        </div>
        <a href="{{ route('admin.offerings.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white shadow-glow transition hover:bg-brand-600">إضافة عرض</a>
    </div>
            ...
            <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                @forelse ($offerings as $offering)
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 text-mist-500">{{ $offering->sort_order }}</td>
                        <td class="px-3 py-2 font-medium text-ink-900 dark:text-ink-50">{{ $offering->title }}</td>
                        <td class="px-3 py-2 text-mist-500">{{ $offering->icon }}</td>
                        <td class="px-3 py-2">
                            <span @class([
                                'rounded-md px-2 py-0.5 text-xs font-semibold',
                                'bg-brand-500/10 text-brand-600' => $offering->is_published,
