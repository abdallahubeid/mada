...
@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">الميزات العامة</h2>
            <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">إدارة عناصر قسم الميزات العامة في صفحة الهبوط.</p>
        </div>
        <a href="{{ route('admin.features.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300">إضافة ميزة</a>
    </div>
            ...
            <tbody class="divide-y divide-mist-100 dark:divide-ink-700">
                @forelse ($features as $feature)
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-mist-500">{{ $feature->sort_order }}</td>
                        <td class="px-4 py-3 font-medium text-ink-900 dark:text-ink-50">{{ $feature->title }}</td>
                        <td class="px-4 py-3 text-mist-500">{{ $feature->icon_key }}</td>
                        <td class="px-4 py-3">
                            <span @class([
                                'rounded-full px-2 py-0.5 text-[10px] font-semibold',
                                'bg-emerald-500/10 text-emerald-600' => $feature->is_published,
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.features.destroy', $feature) }}" onsubmit="return confirm('حذف هذا العنصر؟')">
                                    @csrf
                                    @method('DELETE')
