...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-4 rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
            ...
            <input type="text" dir="ltr" name="icon_key" value="{{ old('icon_key', $feature->icon_key) }}" class="{{ $inputClass }}" placeholder="shield">
        </div>
        <div>
    ...
    <label class="flex items-center gap-2 text-sm text-ink-700 dark:text-mist-200">
        <input type="checkbox" name="is_published" value="1" @checked(old('is_published', $feature->is_published)) class="rounded border-mist-300 text-emerald-500 focus:ring-emerald-400">
        منشور على الموقع
    </label>
