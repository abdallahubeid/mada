...
@section('breadcrumbs')
    <a href="{{ route('admin.tenants') }}" class="text-mist-500 hover:text-emerald-600 dark:text-mist-400 dark:hover:text-emerald-400">المستأجرون</a>
...
@section('content')
    <div x-data="{ modal: null }">
        {{-- Header --}}
        <div class="flex flex-col gap-4 rounded-2xl border border-mist-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between dark:border-ink-600 dark:bg-ink-800">
            <div class="flex items-center gap-4">
                <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-emerald-400/15 font-display text-xl font-bold text-emerald-600 dark:text-emerald-400">
                    {{ mb_substr($tenant['name'], 0, 1) }}
                </span>
                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        <h2 class="font-display text-xl font-bold text-ink-900 dark:text-ink-50">{{ $tenant['name'] }}</h2>
                        <x-admin.status-badge :status="$tenant['status']" />
                    </div>
                    <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">{{ $tenant['slug'] }}.veyra.app · انضم في {{ $tenant['member_since'] }}</p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                @switch($tenant['status'])
                    @case('pending_approval')
                        <button type="button" @click="modal = 'approve'" class="rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition duration-200 hover:bg-emerald-300 active:scale-95">تفعيل</button>
                    ...
                    @case('active')
                        ...
                        @break
                    @case('suspended')
                        <button type="button" @click="modal = 'reactivate'" class="rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition duration-200 hover:bg-emerald-300 active:scale-95">إعادة تفعيل</button>
                        @break
                    @case('cancelled')
            ...
            @if ($tenant['marketing']['persistable'])
                <form method="POST" action="{{ route('admin.tenants.marketing', $tenant['slug']) }}" enctype="multipart/form-data" class="mt-4 space-y-4">
                    ...
                    <label class="flex items-center justify-between gap-4 rounded-xl border border-mist-200 px-4 py-3 dark:border-ink-600">
                        ...
                        <input type="checkbox" name="show_on_marketing" value="1" @checked(old('show_on_marketing', $tenant['marketing']['show_on_marketing'])) class="rounded border-mist-300 text-emerald-500 focus:ring-emerald-400">
                    </label>
                    ...
                    <div>
                        ...
                        @endif
                        <input type="file" name="marketing_logo" accept="image/*" class="w-full rounded-xl border border-mist-200 bg-white px-3 py-2.5 text-sm dark:border-ink-600 dark:bg-ink-900">
                        <input type="text" name="alt_text" value="{{ old('alt_text') }}" placeholder="نص بديل للشعار" class="mt-2 w-full rounded-xl border border-mist-200 bg-white px-3 py-2.5 text-sm dark:border-ink-600 dark:bg-ink-900">
                    </div>

                    <div class="flex justify-end">
                        <button type="submit" class="rounded-xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-glow transition hover:bg-emerald-300">حفظ إعدادات التسويق</button>
                    </div>
                </form>
            @else
        ...
        <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <div class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm lg:col-span-2 dark:border-ink-600 dark:bg-ink-800">
                <div class="flex items-center justify-between">
                    <h3 class="font-display text-base font-semibold text-ink-900 dark:text-ink-50">الاستهلاك مقابل حدود الخطة</h3>
                    <span class="rounded-full bg-mist-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-mist-500 dark:bg-ink-700 dark:text-mist-400">مرجعي · المرحلة 4</span>
                </div>
                ...
                <div class="mt-5 space-y-5">
                    @foreach ($tenant['usage'] as $usage)
                        @php
                            ...
                            $bar = $pct >= 90 ? 'bg-danger-solid' : ($pct >= 75 ? 'bg-amber-500' : 'bg-emerald-400');
                        @endphp
                        <div>
            ...
            <div class="rounded-2xl border border-mist-200 bg-white p-5 shadow-sm dark:border-ink-600 dark:bg-ink-800">
                <h3 class="font-display text-base font-semibold text-ink-900 dark:text-ink-50">مالك الحساب</h3>

                <div class="mt-4 flex items-center gap-3">
                    <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-400/15 font-display text-lg font-bold text-emerald-600 dark:text-emerald-400">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <dd>
                            ...
                            @else
                                <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-medium text-amber-600 dark:text-amber-400">غير موثّق</span>
                            @endif
                        </dd>
                    </div>
                    <div class="flex items-center justify-between">
            ...
            <div class="rounded-2xl border border-mist-200 bg-white shadow-sm lg:col-span-2 dark:border-ink-600 dark:bg-ink-800">
                <div class="flex items-center justify-between border-b border-mist-100 px-5 py-4 dark:border-ink-700">
                    <h3 class="font-display text-base font-semibold text-ink-900 dark:text-ink-50">سجل نشاط المستأجر</h3>
                    <a href="{{ Route::has('admin.audit-log') ? route('admin.audit-log') : '#' }}" class="text-sm font-medium text-emerald-600 hover:underline dark:text-emerald-400">عرض في السجل الكامل</a>
                </div>
                <ul class="divide-y divide-mist-100 dark:divide-ink-700">
                    @foreach ($tenant['audit'] as $entry)
                        <li class="flex items-center justify-between gap-3 px-5 py-3">
                            <div class="flex items-center gap-3">
                                <span class="h-2 w-2 shrink-0 rounded-full bg-mist-300 dark:bg-mist-600"></span>
                                <div class="min-w-0">
            ...
            {{-- Danger zone --}}
            <div class="rounded-2xl border border-danger-solid/30 bg-danger-solid/5 p-5 shadow-sm">
                <h3 class="font-display text-base font-semibold text-danger-solid">منطقة الخطر</h3>
                ...
                <div class="mt-4 space-y-2">
                    ...
                    <button type="button" disabled title="يتوفّر في المرحلة 4" class="flex w-full cursor-not-allowed items-center justify-between gap-2 rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-mist-400 dark:border-ink-600 dark:text-mist-500">
                        ...
                        <span class="rounded-full bg-mist-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-mist-500 dark:bg-ink-700 dark:text-mist-400">Phase 4</span>
                    </button>
                </div>
            </div>
        </div>
