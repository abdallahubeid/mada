...
@section('content')
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-mist-500 dark:text-mist-400">تنبيهات المنصّة على مستوى النظام — حالة القراءة والمسح خاصة بك.</p>
        </div>

        <div class="flex items-center gap-2">
            <button type="button" @click="$dispatch('notif-read-all')" class="rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-ink-700 transition duration-200 hover:border-emerald-400 hover:text-emerald-600 active:scale-[0.98] dark:border-ink-600 dark:text-mist-200 dark:hover:text-emerald-400">
                تحديد الكل كمقروء
            </button>
            <button type="button" @click="$dispatch('notif-clear')" class="rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-mist-600 transition duration-200 hover:border-danger-solid hover:text-danger-solid active:scale-[0.98] dark:border-ink-600 dark:text-mist-300">
                مسح الكل
            </button>
        </div>
    </div>
                    ...
                    <div>
                        ...
                        <ul class="mt-2 space-y-2">
                            @foreach ($group['items'] as $n)
                                <li
                                    x-data="{ read: {{ $n['read'] ? 'true' : 'false' }}, dismissed: false }"
                                    x-show="!dismissed"
                                    x-transition:leave="transition ease-in duration-200"
                                    x-transition:leave-start="opacity-100"
                                    x-transition:leave-end="opacity-0"
                                    @notif-clear.window="dismissed = true"
                                    @notif-read-all.window="read = true"
                                    :class="read ? 'border-mist-200 bg-white dark:border-ink-600 dark:bg-ink-800' : 'border-emerald-400/30 bg-emerald-400/[0.03] dark:bg-ink-800'"
                                    class="group flex items-start gap-3 rounded-2xl border p-4 shadow-sm transition duration-200"
                                >
                                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl {{ $meta[$n['category']]['tone'] }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">{!! $meta[$n['category']]['icon'] !!}</svg>
                                    </span>

                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-2">
                                            <span x-show="!read" class="h-2 w-2 shrink-0 rounded-full bg-emerald-400"></span>
                                        ...
                                        <div class="mt-2 flex items-center gap-3">
                                            ...
                                            @if ($n['target'] && Route::has($n['target']))
                                                <a href="{{ route($n['target']) }}" class="text-xs font-medium text-emerald-600 hover:underline dark:text-emerald-400">الانتقال</a>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex shrink-0 items-center gap-1">
                                        <button
                                            ...
                                            :title="read ? 'تحديد كغير مقروء' : 'تحديد كمقروء'"
                                        >
                                            <svg x-show="!read" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                                            <svg x-show="read" x-cloak xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3.98 8.223A10.477 10.477 0 0 0 1.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.451 10.451 0 0 1 12 4.5c4.756 0 8.773 3.162 10.065 7.498a10.522 10.522 0 0 1-4.293 5.774M6.228 6.228 3 3m3.228 3.228 3.65 3.65m7.894 7.894L21 21m-3.228-3.228-3.65-3.65m0 0a3 3 0 1 0-4.243-4.243m4.243 4.243L9.88 9.88" /></svg>
                                        </button>
                                        <button
                                            type="button"
