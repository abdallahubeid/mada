...
@section('content')
    @php
        $meta = [
            'approval' => ['tone' => 'bg-brand-500/15 text-brand-500 dark:text-brand-300', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />'],
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-2">
                <h2 class="font-display text-2xl font-medium text-ink-900 dark:text-ink-50">مركز الإشعارات</h2>
                @if ($unreadCounts['all'] > 0)
                    <span class="rounded-md bg-brand-500 px-2 py-0.5 text-xs font-bold text-white">{{ $unreadCounts['all'] }} غير مقروء</span>
                @endif
            </div>
        ...
        <div class="flex items-center gap-2">
            <form method="POST" action="{{ route('admin.notifications.read-all') }}">
                @csrf
                <button type="submit" class="rounded-xl border border-mist-200 px-4 py-2 text-sm font-semibold text-ink-700 transition duration-200 hover:border-brand-500 hover:text-brand-600 active:scale-[0.98] dark:border-ink-600 dark:text-mist-200 dark:hover:text-brand-300">
                    تحديد الكل كمقروء
                </button>
            </form>
            <form
        ...
        @foreach ($categories as $key => $cat)
            ...
            @if ($cat['phase4'])
                <span class="inline-flex cursor-not-allowed items-center gap-2 rounded-lg border border-dashed border-mist-200 bg-white px-3 py-1.5 text-sm font-medium text-mist-400 transition-colors dark:border-ink-700 dark:bg-ink-800 dark:text-mist-500" title="يتوفّر في المرحلة 4">
                    {{ $cat['label'] }}
                    <span class="rounded-md bg-mist-100 px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-mist-500 dark:bg-ink-900/60 dark:text-mist-400">Phase 4</span>
                </span>
            @else
                <a
                    ...
                    @class([
                        ...
                        'border-brand-500/30 bg-brand-500/10 text-brand-600 dark:text-brand-300' => $isActive,
                ...
                >
                    ...
                    @if (($unreadCounts[$key] ?? 0) > 0)
                        <span @class([
                            ...
                            'bg-brand-500/20 text-brand-700 dark:text-brand-300' => $isActive,
                            ...
                            @foreach ($group['items'] as $n)
                                ...
                                <li
                                    @class([
                                        ...
                                        'border-brand-500/30 bg-brand-500/[0.03] dark:bg-ink-800' => ! $n['read'],
                                    ])
                                >
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="mt-2 flex items-center gap-3">
                                            ...
                                            @if (! empty($n['target_url']))
                                                <a href="{{ $n['target_url'] }}" class="text-xs font-medium text-brand-600 hover:underline dark:text-brand-300">الانتقال</a>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex shrink-0 items-center gap-1">
                                        <form method="POST" action="{{ route('admin.notifications.toggle-read', $n['id']) }}">
                                            ...
                                            <button
                                                type="submit"
                                                class="rounded-lg p-1.5 text-mist-400 transition hover:bg-mist-100 hover:text-brand-600 active:scale-90 dark:hover:bg-ink-700 dark:hover:text-brand-300"
