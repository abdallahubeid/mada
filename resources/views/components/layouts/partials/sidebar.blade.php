@php
    ...
    $hrManagementItems = [
        ...
        [
            ...
            'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25" />',
        ],
        [
            'label' => '🏢 الأقسام',
    ...
    $financeItems = [
        ...
        [
            'label' => '⚙️ بنود البدلات والاستقطاعات',
        ...
        [
            ...
            'items' => [
                ...
                [
                    ...
                    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664" />',
                ],
                [
                    'label' => '⭐ تقييماتي',
...
>
    <div class="flex h-16 shrink-0 items-center gap-2 border-b border-mist-200 px-6 dark:border-ink-700/70" :class="sidebarCollapsed && 'lg:justify-center lg:px-0'">
        <a
            ...
            title="الرئيسية"
            aria-label="الرئيسية — Veyra ERP"
            @click="closeSidebarDrawer()"
        >
            @if ($tenantLogoUrl)
                <img
                    src="{{ $tenantLogoUrl }}"
                    alt="{{ auth()->user()?->tenant?->name ?? 'Veyra' }}"
            ...
            @else
                ...
                <div class="leading-tight" x-show="! sidebarCollapsed">
                    ...
                    <span class="block text-[10px] font-medium tracking-wide text-mist-500">مساحة العمل</span>
                </div>
            @endif
        </a>
        <button
                                        ...
                                        <div x-data="{ open: {{ $childActive ? 'true' : 'false' }} }" class="space-y-1">
                                            <button
                                                ...
                                                @class([
                                                    ...
                                                    'border-emerald-400 bg-emerald-400/10 text-emerald-600 dark:text-emerald-400' => $childActive,
                                            ...
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0 {{ $childActive ? 'text-emerald-500 dark:text-emerald-400' : 'text-mist-400 group-hover:text-mist-500 dark:text-mist-500' }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                                                    {!! $item['icon'] !!}
                                                </svg>
