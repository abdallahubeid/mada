@foreach ($items as $i => $item)
    ...
    <div class="overflow-hidden rounded-2xl border border-mist-200 bg-white transition duration-200 dark:border-ink-800 dark:bg-ink-800/40">
        ...
        >
            <span class="font-display text-base font-semibold text-ink-900 dark:text-ink-50">{{ $question }}</span>
            <svg
        ...
        <div x-show="open === {{ $i }}" x-collapse x-cloak>
            <p class="px-5 pb-5 text-sm leading-relaxed text-mist-500 dark:text-mist-400">{{ $answer }}</p>
        </div>
    </div>
@endforeach
